 ...
 */
static unsigned int hash_quad(Hash_Table_t* table, unsigned int value)
{
...
bool HashTable_Quad_Insert(Hash_Table_t* table,unsigned int value)
{
    ...
    {
        /* Hash the value */
        unsigned int index = hash_quad(table, value);
        ...
        { 
            ...
            while((HASH_TABLE_ENTRY_OCCUPIED == table->table[index].is_occupied))
            {
                ...
                ++counter;
                index = (index + (i * i)) % (table -> table_size);
                i++;
            }
        }
